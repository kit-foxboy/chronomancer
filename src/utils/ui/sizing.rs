...
//! system. Magic numbers are illegal in themes and you will be purged with a
//! thunder hammer if you use them you filthy FILTHY heretic!
//!
//! # Usage
//!
//! Use `ComponentSize` constants for fixed dimensions (icon sizes, button heights,
//! font sizes). For spacing and padding, use cosmic theme values directly:
//!
//! ```rust,ignore
//! use cosmic::theme;
//!
...
impl ComponentSize {
    ...
    /// Default font size for labels and inputs.
    ///
    /// Use font size constants here so the app UI can be adjusted on the fly
    /// without having to fiddle with individual components.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
